...
class PlayState extends MusicBeatState
{
	...
	public function reloadHealthBarColors() {
		healthBar.createFilledBar(0xFFFF0000,0xFF66FF33);//FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]),
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) + iconP1.width - iconOffset * 0.5 - 150;
		iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (iconP2.width - iconOffset);
		iconP1.y = healthBar.y + iconP1.height - (150 * 1.5);
		iconP2.y = healthBar.y + iconP2.height - (150 * 1.5);
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						MenuWeek.weekCompleted.set(WeekData.weeksList[storyWeek], true);
						...
						FlxG.save.data.weekCompleted = MenuWeek.weekCompleted;
						FlxG.save.flush();
					}
