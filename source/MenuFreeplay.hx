...
class MenuFreeplay extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...songs.length)
			...
			//var icon;
			//icon.sprTracker = songText;:HealthIcon = new HealthIcon()
		...
		disc.scale.x = discIcon.scale.x = 0;
		FlxTween.tween(disc, {'scale.x': 1, y: 480, x: -25}, 0.5, {ease: FlxEase.quartInOut});
		FlxTween.tween(discIcon, {'scale.x': 1}, 0.5, {ease: FlxEase.quartInOut});
		scoreText.alpha = 0;
	...
	function weekIsLocked(name:String):Bool {
		...
		return (!leWeek.startUnlocked && leWeek.weekBefore.length > 0 && (!MenuWeek.weekCompleted.exists(leWeek.weekBefore) || !MenuWeek.weekCompleted.get(leWeek.weekBefore)));
	}
	...
	override function update(elapsed:Float)
	{
		...
		bg.color = side.color = 0xFF2BE6FF;
		checker.color = 0xFF00ADE2;
		...
		var ctrl = FlxG.keys.justPressed.CONTROL || _virtualpad.buttonC.justPressed;

		var shiftMult:Int = 1;
		if(FlxG.keys.pressed.SHIFT || _virtualpad.buttonZ.pressed) shiftMult = 3;

		if(songs.length > 1 && selectable)
		{
			if (upP)
			{
