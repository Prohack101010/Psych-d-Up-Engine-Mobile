		...
		if(!charDropDown.dropPanel.visible) {
			...
			if(char.animationsArray.length > 0) {
				...
				var controlArray:Array<Bool> = [
				    FlxG.keys.justPressed.LEFT || _virtualpad.buttonLeft.pressed,
				    FlxG.keys.justPressed.RIGHT || _virtualpad.buttonRight.pressed,
				    FlxG.keys.justPressed.UP || _virtualpad.buttonUp.pressed,
				    FlxG.keys.justPressed.DOWN || _virtualpad.buttonDown.pressed
				];
				...
				for (i in 0...controlArray.length) {
					if(controlArray[i]) {
						var holdShift = FlxG.keys.pressed.SHIFT || _virtualpad.buttonB.pressed;
						var multiplier = 1;
						if (holdShift)
							multiplier = 10;
